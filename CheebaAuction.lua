-----------------------------------------------------------------------------------------------
-- Client Lua Script for CheebaAuction
...
function CheebaAuction:Init()
    Apollo.RegisterAddon(self)
end

function CheebaAuction:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CheebaAuction.xml")
...
function CheebaAuction:OnDocumentReady()
...
function CheebaAuction:OnWindowClosed(wndHandler, wndControl)
	if wndHandler == wndControl then
		self:OnDestroy()
	end
end

function CheebaAuction:OnDestroy()
...
function CheebaAuction:OnToggleAuctionWindow()
...
function CheebaAuction:OnPlayerCurrencyChanged()
	if self.wndMain and self.wndMain:IsValid() then
...
function CheebaAuction:Initialize()
	if self.wndMain and self.wndMain:IsValid() then
		self.wndMain:Destroy()
	end

	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CheebaAuctionForm", nil, self)
...
function CheebaAuction:InitializeCategories()
	-- Armor Hack (nId == 1)
...
function CheebaAuction:InitializeBuyFilters()
...
function CheebaAuction:OnResizeCategories()
...
function CheebaAuction:OnRefreshBtn(wndHandler, wndControl) -- Also from lua and multiple XML buttons
...
function CheebaAuction:OnHeaderBtnToggle(wndHandler, wndControl)
	self:ToggleAndInitializeBuyOrSell()
end

function CheebaAuction:ToggleAndInitializeBuyOrSell()
...
function CheebaAuction:OnSearchEditBoxChanged(wndHandler, wndControl) -- SearchEditBox
	self.wndMain:FindChild("SearchClearBtn"):Show(string.len(wndHandler:GetText() or "") > 0)
end

function CheebaAuction:OnSearchClearBtn(wndHandler, wndControl)
...
function CheebaAuction:OnSearchCommitBtn(wndHandler, wndControl) -- Also SearchEditBox's WindowKeyReturn
	self.wndMain:FindChild("SearchClearBtn"):SetFocus()
	self:OnRefreshBtn()
end
...
function CheebaAuction:OnSortOptionsUseableToggle(wndHandler, wndControl) -- SortFlyoutStat and SortOptionsTimeLH and etc
	local wndSort = self.wndMain:FindChild("SortContainer")
...
function CheebaAuction:OnFilterFlyoutStatBtn(wndHandler, wndControl) -- FilterFlyoutStat, data is { nNameIdx, eType, nValue }
...
function CheebaAuction:OnFilterEditBoxChanged(wndHandler, wndControl) -- Shared between Rarity and Level Up and Max Buyout CashWindow
...
function CheebaAuction:OnFilterOptionsRarityUpBtn(wndHandler, wndControl)
	self:OnHelperFilterRarityEditBox(wndHandler:GetData(), true)
end

function CheebaAuction:OnFilterOptionsRarityDownBtn(wndHandler, wndControl)
	self:OnHelperFilterRarityEditBox(wndHandler:GetData(), false)
end

function CheebaAuction:OnFilterOptionsLevelUpBtn(wndHandler, wndControl)
	self:OnHelperFilterLevelEditBox(wndHandler:GetData(), true)
end

function CheebaAuction:OnFilterOptionsLevelDownBtn(wndHandler, wndControl)
	self:OnHelperFilterLevelEditBox(wndHandler:GetData(), false)
end

function CheebaAuction:OnHelperFilterLevelEditBox(wndParent, bAdd)
...
function CheebaAuction:OnHelperFilterRarityEditBox(wndParent, bAdd)
...
function CheebaAuction:OnFilterOptionsUseableToggle(wndHandler, wndControl)
...
function CheebaAuction:OnResetFilterBtn(wndHandler, wndControl)
	local wndFilter = self.wndMain:FindChild("FilterContainer")
...
function CheebaAuction:OnFilterOptionsWindowClosed(wndHandler, wndControl)
...
function CheebaAuction:HelperCheckValidValues(wndChanged)
...
function CheebaAuction:GetBuyFilterOptions()
...
function CheebaAuction:InitializeBuy()
...
function CheebaAuction:OnItemAuctionSearchResults(nPage, nTotalResults, tAuctions)
...
function CheebaAuction:BuildListItem(aucCurr, wndParent, bBuyTab)
...
function CheebaAuction:OnRowSelectBtnUncheck(wndHandler, wndControl)
...
function CheebaAuction:OnRowSelectBtnCheck(wndHandler, wndControl)
...
function CheebaAuction:OnItemAuctionBuyOrderSubmitted(bSuccess)
end

function CheebaAuction:OnBidPriceUpBtn(wndHandler, wndControl)
	local wndParent = wndHandler:GetData()
	local aucCurr = wndParent:GetData()
	self:OnHelperBidPriceEditBox(wndParent, aucCurr, true)
end

function CheebaAuction:OnBidPriceDownBtn(wndHandler, wndControl)
	local wndParent = wndHandler:GetData()
	local aucCurr = wndParent:GetData()
	self:OnHelperBidPriceEditBox(wndParent, aucCurr, false)
end

function CheebaAuction:OnHelperBidPriceEditBox(wndParent, aucCurr, bAdd)
...
function CheebaAuction:OnBidResetBtn(wndHandler, wndControl)
	local wndParent = wndHandler:GetData()
	local aucCurr = wndParent:GetData()
...
function CheebaAuction:OnBidPriceAmountChanged(wndHandler, wndControl) -- BidPrice, data is parent
...
function CheebaAuction:HelperValidateBidEditBoxInput()
	local wndParent = self.wndMain:FindChild("BuyContainer"):FindChild("BuyBottomContainer")
	local aucCurr = wndParent:GetData()
...
function CheebaAuction:OnCategoryTopBtnCheck(wndHandler, wndControl) -- CategoryTopBtn
...
function CheebaAuction:OnCategoryTopBtnUncheck(wndHandler, wndControl) -- CategoryTopBtn
	self.wndMain:SetGlobalRadioSel("MarketplaceAuction_CategoryMidBtn_GlobalRadioGroup", -1)
...
function CheebaAuction:OnCategoryMidBtnCheck(wndHandler, wndControl)
...
function CheebaAuction:OnCategoryMidBtnUncheck(wndHandler, wndControl)
...
function CheebaAuction:OnSellListItemCheck(wndHandler, wndControl)
...
function CheebaAuction:OnSellListItemUncheck(wndHandler, wndControl)
	if wndHandler ~= wndControl or not wndHandler:FindChild("ListItemTitle") then
		return
	end
...
function CheebaAuction:OnCreateBidInputBoxChanged(wndHandler, wndControl)
	self:ValidateSellOrder()
end

function CheebaAuction:OnCreateBuyoutInputBoxChanged(wndHandler, wndControl)
	self:ValidateSellOrder()
end

function CheebaAuction:ValidateSellOrder() -- CreateSellOrderBtn data is oItemInstance
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CheebaAuction:OnItemAuctionSellOrderSubmitted(wndHandler, wndControl)
	self.wndMain:FindChild("SellLeftSideItemList"):DestroyChildren()
	self.wndMain:FindChild("SellRightSide"):Show(false)
	self:ToggleAndInitializeBuyOrSell()
end

function CheebaAuction:OnBuySearchListItemMouseEnter(wndHandler, wndControl) -- Build on mouse enter and not every hit to save computation time
...
function CheebaAuction:OnBuySearchListItemMouseExit(wndHandler, wndControl)
	if wndHandler == wndControl and wndHandler:FindChild("ListIcon") then
		wndHandler:FindChild("ListIcon"):SetTooltipDoc(nil)
	end
end

function CheebaAuction:OnSellListItemMouseEnter(wndHandler, wndControl) -- ListItem, data should be an item
...
function CheebaAuction:OnPostItemAuctionResult(eAuctionPostResult, aucCurr)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CheebaAuction:OnItemAuctionBidResult(eAuctionBidResult, aucCurr)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CheebaAuction:OnItemCancelResult(eAuctionEventType, aucCurr)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CheebaAuction:OnPostCustomMessage(strMessage, bResultOK, nDuration)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CheebaAuction:OnPostResultTimer()
	if self.wndMain and self.wndMain:IsValid() then
		Apollo.StopTimer("PostResultTimer")
...
function CheebaAuction:OnItemAuctionWon(aucCurrent)
...
function CheebaAuction:OnItemAuctionOutbid(aucCurrent)
	local bValidItem = aucCurrent and aucCurrent:GetItem()
	local strItemName = bValidItem and aucCurrent:GetItem():GetName() or ""
...
function CheebaAuction:OnItemAuctionExpired(aucCurrent)
...
function CheebaAuction:OnBuySearchFirstBtn(wndHandler, wndControl)
	self.fnLastSearch(0)
end

function CheebaAuction:OnBuySearchPrevBtn(wndHandler, wndControl)
	if self.nCurPage > 0 then
		self.fnLastSearch(self.nCurPage - 1)
	end
end

function CheebaAuction:OnBuySearchNextBtn(wndHandler, wndControl)
	self.fnLastSearch(self.nCurPage + 1)
end

function CheebaAuction:OnBuySearchLastBtn(wndHandler, wndControl)
...
function CheebaAuction:OnPostResultNotificationClick(wndHandler, wndControl)
	if wndHandler == wndControl then
		wndHandler:Show(false)
	end
end

function CheebaAuction:OnOpenMarketListingsBtn(wndHandler, wndControl)
	Event_FireGenericEvent("InterfaceMenu_ToggleCheebaListings")
end

function CheebaAuction:OnOwnedItemAuctions(tAuctions) -- From MarketplaceLib.RequestOwnedItemAuctions()
	local nCount = 0
...
function CheebaAuction:UpdateOrderLimit(nCount)
	if nCount < 0 then
		self.nOwnedAuctionCount = 0
	else
...
function CheebaAuction:OnGenericEditBoxMouseDown(wndHandler, wndControl)
	wndHandler:SetStyleEx("SkipZeroes", false)
end

function CheebaAuction:OnGenericEditBoxLoseFocus(wndHandler, wndControl)
	wndHandler:SetStyleEx("SkipZeroes", true)
end

function CheebaAuction:HelperBuildItemTooltip(wndArg, itemSource, itemModData)
...
function CheebaAuction:HelperFormatTimeString(oExpirationTime)
	local strResult = ""
...
function CheebaAuction:LoadByName(strForm, wndParent, strCustomName)
...
local CheebaAuctionInst = CheebaAuction:new()
CheebaAuctionInst:Init()
